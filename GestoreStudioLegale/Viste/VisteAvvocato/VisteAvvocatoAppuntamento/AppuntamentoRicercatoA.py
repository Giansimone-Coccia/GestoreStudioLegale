...
class AppuntamentoRicercatoA(QMainWindow):
    ...
    def getDatiAp(self):
        ...
        i += 1
        self.grid.addWidget(self.tool.createButton("Modifica", lambda: self.vistaAggiorna.show(), method2=self.close),
                            i, 1)
        self.grid.addWidget(
            self.tool.createButton("Elimina", lambda: self.rimuoviAppuntamento(
                self.appuntamentoTrovato.getDatiAppuntamento()['ID']), method2=self.close), i, 2)
