...
from GestoreStudioLegale.Viste.VisteAvvocato.VisteAvvocatoAppuntamento.RicercaAppuntamentoA import RicercaAppuntamentoA
from GestoreStudioLegale.Viste.VisteAvvocato.VisteAvvocatoAppuntamento.VistaEliminaAppuntamentoA import VistaEliminaAppuntamentoA

from GestoreStudioLegale.Viste.VisteAvvocato.VisteAvvocatoAppuntamento.VisteInserisciAppuntamentoA import VisteInserisciAppuntamentoA
...
class VistaHomeAppuntamentiA(QMainWindow):
    ...
    def aggiornaAppuntamento(self, appuntamento):
        from GestoreStudioLegale.Viste.VisteAvvocato.VisteAvvocatoAppuntamento.VistaAggiornaAppuntamentoA import \
            VistaAggiornaAppuntamentoA
                ...
                for appuntamento in avvocato.appuntamentiAvvocato:
                    for cliente in avvocato.clienti:
                        if cliente.getDatiCliente()['Id'] == appuntamento.Cliente.getDatiCliente()['Id']:
                            ...
                            datIn = appuntamento.getDatiAppuntamento()['Data e Ora Inizio'].strftime(
                                "%m/%d/%Y, %H:%M:%S")
                            datFin = appuntamento.getDatiAppuntamento()['Data e Ora Fine'].strftime(
                                "%m/%d/%Y, %H:%M:%S")
                            label.setText(
                            ...
                            i += 1
                            self.grid.addWidget(self.tool.createButton("Modifica", lambda checked,
                                                                                          a=appuntamento: self.aggiornaAppuntamento(
                                a)), i, 1)
                            self.grid.addWidget(
                                self.tool.createButton("Elimina", lambda checked, a=appuntamento.getDatiAppuntamento()[
